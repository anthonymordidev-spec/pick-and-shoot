-- Pick & Shoot: synchronized multi-player tournament engine.
-- Run after the existing room/custom-rules migrations.
...
as $$
  select case when ruleset='custom' then custom_rules else jsonb_build_object('items', jsonb_build_array(jsonb_build_object('id','rock','label','Rock','glyph','✊'),jsonb_build_object('id','paper','label','Paper','glyph','✋'),jsonb_build_object('id','scissors','label','Scissors','glyph','✌')), 'beats', jsonb_build_object('rock',jsonb_build_array('scissors'),'paper',jsonb_build_array('rock'),'scissors',jsonb_build_array('paper'))) end
...
as $$
declare room_row public.rooms; t public.tournaments; rec record; idx integer:=0; deadline timestamptz; p1 uuid; p2 uuid; max_stage integer;
begin
 ...
 if (select count(*) from public.room_players where room_id=p_room_id and eliminated=false)<2 then raise exception 'NOT_ENOUGH_PLAYERS'; end if;
 ...
 for rec in select user_id,display_name from public.room_players where room_id=p_room_id and eliminated=false order by random() loop
   idx:=idx+1;
   if p1 is null then p1:=rec.user_id; continue; end if;
   p2:=rec.user_id;
   insert into public.tournament_pairings(tournament_id,stage_number,pair_index,player_a_id,player_b_id,against_randomizer)
   values(t.id,1,idx,p1,p2,false)
   returning id into p1; -- temporary holder for pairing id
   deadline:=now()+interval '5 seconds';
   insert into public.tournament_rounds(pairing_id,round_number,phase,pick_deadline) values(p1,1,'picking',deadline);
   p1:=null;
 end loop;
 -- If odd humans, the unpaired human plays the Randomizer.
 if (select count(*) from public.room_players where room_id=p_room_id and eliminated=false)%2=1 then
   select user_id into p2 from public.room_players where room_id=p_room_id and eliminated=false and user_id not in (select unnest(array[]::uuid[])) order by random() limit 1;
   -- choose a human not already assigned in this stage
   select rp.user_id into p2 from public.room_players rp
   where rp.room_id=p_room_id and rp.eliminated=false
   and not exists(select 1 from public.tournament_pairings tp where tp.tournament_id=t.id and tp.stage_number=1 and (tp.player_a_id=rp.user_id or tp.player_b_id=rp.user_id))
   limit 1;
   idx:=idx+1;
   insert into public.tournament_pairings(tournament_id,stage_number,pair_index,player_a_id,player_b_id,against_randomizer)
   values(t.id,1,idx,p2,null,true) returning id into p1;
   insert into public.tournament_rounds(pairing_id,round_number,phase,pick_deadline) values(p1,1,'picking',now()+interval '5 seconds');
 end if;
...
declare r public.tournament_rounds; p public.tournament_pairings; t public.tournaments; room_id_value uuid;
begin
 if auth.uid() is null then raise exception 'AUTH_REQUIRED'; end if;
 ...
 room_id_value:=t.room_id;
 if auth.uid()<>p.player_a_id and auth.uid()<>p.player_b_id then raise exception 'NOT_IN_MATCH'; end if;
 if not public._pns_valid_choice(room_id_value,p_choice) then raise exception 'INVALID_CHOICE'; end if;
...
returns jsonb language plpgsql security definer set search_path=public as $$
declare r public.tournament_rounds; p public.tournament_pairings; t public.tournaments; room_id_value uuid; c1 text; c2 text; rules jsonb; beats jsonb; winner uuid; choice_count integer; random_choice text; options text[]; target integer; wins integer; now_ts timestamptz:=now();
begin
 if auth.uid() is null then raise exception 'AUTH_REQUIRED'; end if;
 select * into r from public.tournament_rounds where id=p_round_id for update; if not found then raise exception 'ROUND_NOT_FOUND'; end if;
 select * into p from public.tournament_pairings where id=r.pairing_id; select * into t from public.tournaments where id=p.tournament_id; room_id_value:=t.room_id;
 if not public.is_room_member(room_id_value) then raise exception 'NOT_IN_ROOM'; end if;
 if r.phase='finished' then return jsonb_build_object('resolved',true,'phase','finished','winner_user_id',r.winner_user_id,'choices',r.resolved_choices); end if;
 select count(*) into choice_count from public.tournament_choices where round_id=r.id;
 if p.against_randomizer then
   if choice_count<1 and now_ts<r.pick_deadline then return jsonb_build_object('resolved',false,'phase','picking'); end if;
 else
   if choice_count<2 and now_ts<r.pick_deadline then return jsonb_build_object('resolved',false,'phase','picking'); end if;
 end if;
 rules:=public._pns_rules_for_room(room_id_value); beats:=rules->'beats';
 ...
 if p.against_randomizer then
   random_choice:=options[1+floor(random()*array_length(options,1))::int]; c2:=random_choice;
 else
 ...
 elsif coalesce(beats->c1,'[]'::jsonb) ? c2 then winner:=p.player_a_id;
 elsif p.against_randomizer then winner:=null; -- randomizer wins only when it actually beats the human
 else winner:=p.player_b_id; end if;
 update public.tournament_rounds set phase='finished',winner_user_id=winner,resolved_choices=jsonb_build_object(p.player_a_id::text,c1,coalesce(p.player_b_id,'00000000-0000-0000-0000-000000000000')::text,c2),resolved_at=now_ts where id=r.id;
 target:=ceil(t.best_of::numeric/2)::int;
 select count(*) into wins from public.tournament_rounds rr where rr.pairing_id=p.id and rr.phase='finished' and rr.winner_user_id=p.player_a_id;
 if wins>=target then update public.tournament_pairings set status='finished',winner_user_id=p.player_a_id,randomizer_won=false,finished_at=now_ts where id=p.id;
 elsif not p.against_randomizer then
   select count(*) into wins from public.tournament_rounds rr where rr.pairing_id=p.id and rr.phase='finished' and rr.winner_user_id=p.player_b_id;
   if wins>=target then update public.tournament_pairings set status='finished',winner_user_id=p.player_b_id,randomizer_won=false,finished_at=now_ts where id=p.id; end if;
 elsif p.against_randomizer and winner is null then update public.tournament_pairings set status='finished',winner_user_id=null,randomizer_won=true,finished_at=now_ts where id=p.id;
 end if;
 if (select status from public.tournament_pairings where id=p.id)='active' then
 ...
 return jsonb_build_object('resolved',true,'phase','finished','winner_user_id',winner,'choices',jsonb_build_object(p.player_a_id::text,c1,coalesce(p.player_b_id,'00000000-0000-0000-0000-000000000000')::text,c2));
end; $$;
...
returns jsonb language plpgsql security definer set search_path=public as $$
declare t public.tournaments; survivors uuid[]; survivor uuid; idx integer:=0; p1 uuid; pairing_id uuid; humans integer; next_stage integer;
begin
 ...
 humans:=coalesce(array_length(survivors,1),0);
 if humans<=1 then
 ...
 foreach survivor in array survivors loop
   ...
   idx:=idx+1; insert into public.tournament_pairings(tournament_id,stage_number,pair_index,player_a_id,player_b_id,against_randomizer) values(t.id,next_stage,idx,p1,survivor,false) returning id into pairing_id;
   insert into public.tournament_rounds(pairing_id,round_number,phase,pick_deadline) values(pairing_id,1,'picking',now()+interval '5 seconds'); p1:=null;
 end loop;
 if p1 is not null then
   idx:=idx+1; insert into public.tournament_pairings(tournament_id,stage_number,pair_index,player_a_id,player_b_id,against_randomizer) values(t.id,next_stage,idx,p1,null,true) returning id into pairing_id;
...
returns jsonb language plpgsql security definer set search_path=public as $$
declare t public.tournaments; room_row public.rooms; human_count integer; pairings jsonb; your_id uuid:=auth.uid(); your_pairing uuid; current_round uuid;
begin
 if auth.uid() is null or not public.is_room_member(p_room_id) then raise exception 'NOT_IN_ROOM'; end if;
 ...
 select jsonb_agg(jsonb_build_object(
   ...
   'resolvedChoices',r.resolved_choices,'roundWinnerUserId',r.winner_user_id,
   ...
   'scoreB',(select count(*) from public.tournament_rounds rr where rr.pairing_id=p.id and rr.phase='finished' and rr.winner_user_id=p.player_b_id)
